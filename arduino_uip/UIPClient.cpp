  ...
  if (u && !(u->state & (UIP_CLIENT_CLOSE | UIP_CLIENT_REMOTECLOSED)))
    {
      ...
      if (u->packets_out[p] == NOBLOCK)
        {
newpacket:
          u->packets_out[p] = Enc424J600Network::allocBlock(UIP_SOCKET_DATALEN);
          if (u->packets_out[p] == NOBLOCK)
            {
#if UIP_ATTEMPTS_ON_WRITE > 0
              if ((--attempts)>0)
...
UIPClient::_available(uip_userdata_t *u)
{
  ...
  for (uint8_t i = 0; i < UIP_SOCKET_NUMPACKETS; i++)
    {
      len += Enc424J600Network::blockSize(u->packets_in[i]);
    }
  return len;
}
  ...
  if (*this)
    {
      ...
      do
        {
          read = Enc424J600Network::readPacket(data->packets_in[0],0,buf+size-remain,remain);
          if (read == Enc424J600Network::blockSize(data->packets_in[0]))
            {
              remain -= read;
            ...
            {
              Enc424J600Network::resizeBlock(data->packets_in[0],read);
              break;
            }
        }
      while(remain > 0);
  ...
  if (*this)
    {
      if (data->packets_in[0] != NOBLOCK)
        {
          uint8_t c;
          Enc424J600Network::readPacket(data->packets_in[0],0,&c,1);
          return c;
        }
    }
  return -1;
              ...
              if (u->packets_out[1] == NOBLOCK)
                {
                  ...
                  if (send_len > 0)
                    {
                      Enc424J600Network::resizeBlock(u->packets_out[0],0,send_len);
                    }
                }
              else
                send_len = Enc424J600Network::blockSize(u->packets_out[0]);
              if (send_len > 0)
                {
...
UIPClient::_flushBlocks(memhandle* block)
{
  for (uint8_t i = 0; i < UIP_SOCKET_NUMPACKETS; i++)
    {
      Enc424J600Network::freeBlock(block[i]);
      block[i] = NOBLOCK;
    }
}
