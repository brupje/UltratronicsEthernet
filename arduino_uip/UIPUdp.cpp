  ...
  if (_uip_udp_conn)
    {
      ...
      _uip_udp_conn=NULL;
      Enc424J600Network::freeBlock(appdata.packet_in);
      Enc424J600Network::freeBlock(appdata.packet_next);
      Enc424J600Network::freeBlock(appdata.packet_out);
      memset(&appdata,0,sizeof(appdata));
    }
}
  ...
  if (_uip_udp_conn)
    {
      if (appdata.packet_out == NOBLOCK)
        {
          appdata.packet_out = Enc424J600Network::allocBlock(UIP_UDP_MAXPACKETSIZE);
...
UIPUDP::endPacket()
{
  if (_uip_udp_conn && appdata.packet_out != NOBLOCK)
    {
      appdata.send = true;
      Enc424J600Network::resizeBlock(appdata.packet_out,0,appdata.out_pos);
...
UIPUDP::write(const uint8_t *buffer, size_t size)
{
  if (appdata.packet_out != NOBLOCK)
    {
      size_t ret = Enc424J600Network::writePacket(appdata.packet_out,appdata.out_pos,(uint8_t*)buffer,size);
      appdata.out_pos += ret;
      return ret;
    }
  return 0;
...
#endif
  int size = Enc424J600Network::blockSize(appdata.packet_in);
#ifdef UIPETHERNET_DEBUG_UDP
  if (appdata.packet_in != NOBLOCK)
    {
      Serial.print(F(", size: "));
...
{
  UIPEthernetClass::tick();
  return Enc424J600Network::blockSize(appdata.packet_in);
}
...
UIPUDP::read(unsigned char* buffer, size_t len)
{
  ...
  if (appdata.packet_in != NOBLOCK)
    {
      memaddress read = Enc424J600Network::readPacket(appdata.packet_in,0,buffer,len);
      if (read == Enc424J600Network::blockSize(appdata.packet_in))
        {
          Enc424J600Network::freeBlock(appdata.packet_in);
          appdata.packet_in = NOBLOCK;
        }
      else
        Enc424J600Network::resizeBlock(appdata.packet_in,read);
      return read;
    }
  return 0;
}
...
UIPUDP::peek()
{
  ...
  if (appdata.packet_in != NOBLOCK)
    {
      unsigned char c;
      if (Enc424J600Network::readPacket(appdata.packet_in,0,&c,1) == 1)
        return c;
    }
  return -1;
}
          ...
          if (data->packet_next == NOBLOCK)
            {
              ...
              data->packet_next = Enc424J600Network::allocBlock(ntohs(UDPBUF->udplen)-UIP_UDPH_LEN);
              ...
              if (data->packet_next != NOBLOCK)
                {
                  ...
                  Serial.print(F(", size: "));
                  Serial.println(Enc424J600Network::blockSize(data->packet_next));
    #endif
                }
            }
        }
      if (uip_poll() && data->send)
        {
          ...
          Serial.print(F(", size: "));
          Serial.println(Enc424J600Network::blockSize(data->packet_out));
#endif
